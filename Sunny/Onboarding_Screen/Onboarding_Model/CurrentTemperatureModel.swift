...
struct CurrentTemperatureModel {
    ...
    func fillView(_ current: CurrentWeatherView?, _ today: TodayConditionView?, _ data: CurrentWeatherModel?, _ converter: Converter?, _ timeConverter: TimeConverter?, _ timeZone: Int?) {
        ...
        current.dateLabel.text = timeConverter.convertToDayMonthNumber(data.asOf, timeZone)
...
//        current.temperatureLabel.text = converter.convertTemperature(data.temperature ?? 0.0)
        ...
        if data.temperatureApparent >= 0.1 {
            today.feelsLike.parameterImage.image = ConditionImages.thermometerHot
        } else {
            today.feelsLike.parameterImage.image = ConditionImages.thermometerCold
        }
        ...
        if data.precipitationIntensity == 0.0 {
            today.rain.parameterImage.image = ConditionImages.umbrella
        } else {
            today.rain.parameterImage.image = ConditionImages.rainfall
        }
